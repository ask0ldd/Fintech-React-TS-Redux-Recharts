...
const Crypto = ({name, owned, growth, dashes} : ICryptoAsset) => {
    return(
        <article className={dashes === true ? 'crypto__container2' : 'crypto__smallerContainer2'}>
            ...
            <div className={owned >= 0 ? 'owned__positive2' : 'owned__negative2'}><span className='delayed__text'>{Formatter.addCurrencySignToAmount('$', owned)}</span><div className='default__loading loading__mg2'></div></div>
